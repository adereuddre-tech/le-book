...
let dom=mk(null);
let w=dom.window;w.__lev=+(opt('lev')||1);w=dom.window;let d=w.document;const $=s=>d.querySelector(s);
...
const SIG=[];while(steps++<3000&&!done){
 ...
 if($('#send')){w.eval(`try{const R=recoBook();S.k=R.k.map(v=>Math.max(-S.maxk,Math.min(S.maxk,Math.round(v*(window.__lev||1)))));drawRows();renderRisk();refreshSend()}catch(e){window.__e=e.message}`);
...
const chk=w.eval('window.__chk')||[];const res={resumed,bad:w.eval('window.__bad||0'),npo:chk.filter(c=>c.si===0).length,pbar:chk.reduce((a,c)=>a+c.p,0)/Math.max(1,chk.length),seed,cfg,sage,done,steps,errs:errs.slice(0,5),nerr:errs.length,stats,over:w.eval('S&&S.over'),tails:w.eval('JSON.stringify((S.tails||[]).map(x=>x.id+":"+(x.f*100).toFixed(0)))'),cards:w.eval('JSON.stringify((S.cards||{}).log||[])').length,red:w.eval('(S.cards||{}).r||0'),riv:w.eval('JSON.stringify(S.rivals.map(r=>[+(r.cum).toFixed(2),(r.vqh||[]).map(v=>+v.toFixed(2)).join("/")]))'),idx:w.eval('S.idx'),q:w.eval('S&&S.q'),nav:w.eval('S&&S.nav')};
