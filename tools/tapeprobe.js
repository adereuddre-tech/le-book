...
function playGame(o){
  ...
  /* sonde ruban : à chaque rendu de la barre d'état, ce que dit le ruban contre ce que dit la tuile */
  w.eval(`window.__tp=[];(function(){const SB=statusBar;window.statusBar=function(done){const r=SB.apply(this,arguments);
   try{if(S.tape&&S.tape.pts&&S.tape.pts.length>2&&(S.phase==='events'||S.phase==='debrief')){
    const y0=Math.floor(S.tape.y===undefined?0:S.tape.y*4);let base=1;
    const lat=(S.phase==='events'&&S.live)?liveRet():0,tile=S.idx*(1+lat)/base*100,tape=S.tape.pts[S.tape.pts.length-1];
    window.__tp.push({ph:S.phase,q:S.q,mid:!!S.bigTape,tile,tape,d:tape-tile,tq:S.tape.q,live:!!S.live,lat,idx:S.idx,base,ev:S.evIdx,n:S.tape.pts.length,y:S.tape.y})}}catch(e){window.__tp.push({err:e.message})}return r}})()`);
  ...
  try{r.tp=JSON.parse(w.eval('JSON.stringify(window.__tp)'))}catch(e){}
  w.close();
