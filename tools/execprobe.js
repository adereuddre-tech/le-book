...
function playGame(o){
  ...
  w.eval(`window.__ex={q:[],ev:[]};(function(){
   const SE=screenExec;window.screenExec=function(){const tc0=S.pendingTC*S.nav;const r=SE.apply(this,arguments);
     let tot=0;for(let i=0;i<N;i++){const d=S.k[i]-S.k0[i];if(Math.abs(d)>1e-9)tot+=tcost(d,i).cost}
     window.__ex.q.push({q:S.q,nav:S.nav,bill:tot,tc0});return r};
   const RQ=resolveQuarter;window.resolveQuarter=function(){
     const e=window.__ex.q[window.__ex.q.length-1]||{};e.tcPaid=S.pendingTC*S.nav;e.fees=S.mgrQ&&S.mgrQ.fees;
     e.qEvM=S.qEvM;e.tcMultQ=S.tcMultQ;e.slip=S.execSlipQ||0;e.leak=!!S.leakQ;e.lp=S.lp;e.rc=S.rc;return RQ.apply(this,arguments)};
  })()`);
  ...
  try{r.ex=JSON.parse(w.eval('JSON.stringify(window.__ex)'))}catch(e){}
  w.close();
